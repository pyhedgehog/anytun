/**
 *  \file 
 *  \brief Implementation of SyncClient.
 */
/*
 *  anytun
 *
...
void SyncClient::run()
{
  ...
  for(;;) {
    try {
      ...
      if (!connected) { cLog.msg(Log::PRIO_NOTICE) << "sync: connected to " << hostname_ <<":"<< port_; }
      connected = true;
...
void SyncClient::readExactly(SyncTcpConnection::proto::socket& socket, size_t toread, std::iostream& result)
{
  ...
  while (toread > hasread) {
    boost::array<char, 1> buf; // TODO read using much bigger buffers
